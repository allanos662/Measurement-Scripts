import logging
import time
#myppms=QdInstrument('DynaCool',ip_address,remote=False)
import clr
...
dllpath=r'C:\Users\tfrlab\Documents\LabVIEW VIs\QD LV-MultiVu bridge code\QDInstrument'
clr.AddReference(dllpath)

#if clr.FindAssembly(dllpath) is None:
#    logger.exception('\n\tCould not find QDInstrument.dll')
#else:
#    logger.exception('\n\tFound QDInstrument.dll at {}'.format(clr.FindAssembly(dllpath)))
...
class QdInstrument:
    ...
    def __init__(self, instrument_type,remote=False):
    ...
    def ReadSDO_F32(self,nodeID,index,sub_index):
        """Returns the instrument temperature in Kelvin.
        Parameters are from:
        SetTemperature(double Temperature, double Rate, QDInstrumentBase.TemperatureApproach Approach)
        """
        return self.qdi_instrument.ReadSDO_F32(nodeID,index,sub_index) 

    def WriteSDO_F32(self,nodeID,index,sub_index,value):
        """Returns the instrument temperature in Kelvin.
        Parameters are from:
        SetTemperature(double Temperature, double Rate, QDInstrumentBase.TemperatureApproach Approach)
        """
        return self.qdi_instrument.WriteSDO_F32(nodeID,index,sub_index,value) 

    def WriteSDO_U8(self,nodeID,index,sub_index,value):
        """Returns the instrument temperature in Kelvin.
        Parameters are from:
        SetTemperature(double Temperature, double Rate, QDInstrumentBase.TemperatureApproach Approach)
        """
        return self.qdi_instrument.WriteSDO_U8(nodeID,index,sub_index,value)

    def WriteSDO_String(self,nodeID,index,sub_index,value):
        """Returns the instrument temperature in Kelvin.
        Parameters are from:
        SetTemperature(double Temperature, double Rate, QDInstrumentBase.TemperatureApproach Approach)
        """
        return self.qdi_instrument.WriteSDO_String(nodeID,index,sub_index,value) 

    def Start_ADR_Temp(self):
        self.WriteSDO_F32(21,24580,6,0.01)
        self.WriteSDO_F32(21,24580,7,100)
        self.WriteSDO_F32(21,24580,8,3)
        self.WriteSDO_U8(21,24580,4,0)
        self.WriteSDO_U8(21,24580,3,1)
        self.WriteSDO_U8(21,26627,1,3)
        self.WriteSDO_String(21,26627,2,'ADR001')
        time.sleep(5)

    def Stop_ADR_Temp(self):
        self.WriteSDO_U8(21,24580,3,0)

    def getADR_Thermometer_R(self):
        self.ReadSDO_F32(21,24580,1)
    
    def getADRTemp(self):
        return self.ReadSDO_F32(21,int("6030",16),1)
        #return self.ReadSDO_F32(21,int("6014", 16),x,int("13", 16))
